...
define('zapp_user_main', ['dart_sdk', 'flutter_sdk'], (function load__zapp_user_main(dart_sdk, flutter_sdk) {
  ...
  var recipe_details_page = Object.create(dart.library);
  ...
  var T = {
    ...
    BuildContextToRecipeDetailsPage: () => (T.BuildContextToRecipeDetailsPage = dart.constFn(dart.fnType(recipe_details_page.RecipeDetailsPage, [framework.BuildContext])))(),
  ...
  var I = [
    ...
    "file:///zapp/project/lib/recipe_details_page.dart",
  ...
  main.MyApp = class MyApp extends framework.StatelessWidget {
    build(context) {
      return new app.MaterialApp.new({home: new main.RecipeInfoApp.new()});
    }
  ...
  main.RecipeInfoApp = class RecipeInfoApp extends framework.StatefulWidget {
    createState() {
      return new main._RecipeInfoAppState.new();
    }
    static ['_#new#tearOff']() {
      return new main.RecipeInfoApp.new();
    }
  };
  (main.RecipeInfoApp.new = function() {
    main.RecipeInfoApp.__proto__.new.call(this);
    ;
  }).prototype = main.RecipeInfoApp.prototype;
  dart.addTypeTests(main.RecipeInfoApp);
  dart.addTypeCaches(main.RecipeInfoApp);
  dart.setMethodSignature(main.RecipeInfoApp, () => ({
    __proto__: dart.getMethods(main.RecipeInfoApp.__proto__),
    createState: dart.fnType(main._RecipeInfoAppState, [])
  }));
  dart.setLibraryUri(main.RecipeInfoApp, I[0]);
  var _recipeData = dart.privateName(main, "_recipeData");
  var _isLoading = dart.privateName(main, "_isLoading");
  var EdgeInsets_bottom = dart.privateName(edge_insets, "EdgeInsets.bottom");
  var EdgeInsets_right = dart.privateName(edge_insets, "EdgeInsets.right");
  var EdgeInsets_top = dart.privateName(edge_insets, "EdgeInsets.top");
  var EdgeInsets_left = dart.privateName(edge_insets, "EdgeInsets.left");
  main._RecipeInfoAppState = class _RecipeInfoAppState extends framework.State$(main.RecipeInfoApp) {
    fetchRecipeInfo(cuisine) {
      return async.async(dart.void, (function* fetchRecipeInfo() {
        this.setState(dart.fn(() => {
          this[_isLoading] = true;
        }, T.VoidTovoid()));
        let url = "https://www.themealdb.com/api/json/v1/1/filter.php?a=" + cuisine;
        let response = (yield http.get(core.Uri.parse(url)));
        if (response.statusCode === 200) {
          this.setState(dart.fn(() => {
            this[_recipeData] = T.ListN().as(dart.dsend(convert.json.decode(response.body), '_get', ["meals"]));
    ...
    fetchRecipeDetails(mealId) {
      return async.async(T.MapOfString$dynamic(), function* fetchRecipeDetails() {
        let url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=" + mealId;
        let response = (yield http.get(core.Uri.parse(url)));
        if (response.statusCode === 200) {
          let data = dart.dsend(dart.dsend(convert.json.decode(response.body), '_get', ["meals"]), '_get', [0]);
          return T.FutureOrOfMapOfString$dynamic().as(data);
        } else {
          dart.throw(core.Exception.new("Falha ao buscar detalhes da receita"));
        }
      });
    }
    showRecipeDetails(context, recipe) {
      return async.async(dart.void, (function* showRecipeDetails() {
        let mealId = recipe[$_get]("idMeal");
        let recipeDetails = (yield this.fetchRecipeDetails(core.String.as(mealId)));
        navigator.Navigator.push(dart.dynamic, context, new page.MaterialPageRoute.new({builder: dart.fn(context => new recipe_details_page.RecipeDetailsPage.new({recipe: recipeDetails}), T.BuildContextToRecipeDetailsPage())}));
      }).bind(this));
    }
    build(context) {
      return new scaffold.Scaffold.new({appBar: new app_bar.AppBar.new({title: new text.Text.new("Livro de Receitas")}), body: new basic.Center.new({child: new basic.Padding.new({padding: C[1] || CT.C1, child: new basic.Column.new({mainAxisAlignment: flex.MainAxisAlignment.center, children: T.JSArrayOfWidget().of([new (T.DropdownButtonOfString()).new({hint: new text.Text.new("Selecione uma cozinha"), value: this.selectedCuisine, onChanged: dart.fn(newValue => {
                    this.setState(dart.fn(() => {
                      this.selectedCuisine = newValue;
                    }, T.VoidTovoid()));
                  }, T.StringNTovoid()), items: this.cuisines[$map](T.DropdownMenuItemOfString(), dart.fn(cuisine => new (T.DropdownMenuItemOfString()).new({value: cuisine, child: new text.Text.new(cuisine)}), T.StringToDropdownMenuItemOfString()))[$toList]()}), new basic.SizedBox.new({height: 20}), new elevated_button.ElevatedButton.new({onPressed: dart.fn(() => {
                    if (this.selectedCuisine != null) {
                      this.fetchRecipeInfo(dart.nullCheck(this.selectedCuisine));
                    }
                  }, T.VoidTovoid()), child: new text.Text.new("Buscar")}), new basic.SizedBox.new({height: 20}), this[_isLoading] ? new progress_indicator.CircularProgressIndicator.new() : this[_recipeData] != null ? new basic.Expanded.new({child: new scroll_view.ListView.builder({itemCount: dart.nullCheck(this[_recipeData])[$length], itemBuilder: dart.fn((context, index) => new gesture_detector.GestureDetector.new({onTap: dart.fn(() => {
                        this.showRecipeDetails(context, T.MapOfString$dynamic().as(dart.nullCheck(this[_recipeData])[$_get](index)));
                      }, T.VoidTovoid()), child: new card.Card.new({child: new list_tile.ListTile.new({title: new text.Text.new(core.String.as(dart.dsend(dart.nullCheck(this[_recipeData])[$_get](index), '_get', ["strMeal"]))), leading: new image.Image.network(core.String.as(dart.dsend(dart.nullCheck(this[_recipeData])[$_get](index), '_get', ["strMealThumb"])), {width: 50, height: 50, fit: box_fit.BoxFit.cover})})})}), T.BuildContextAndintToGestureDetector())})}) : new text.Text.new("Nenhuma receita encontrada.")])})})})});
    }
    static ['_#new#tearOff']() {
      return new main._RecipeInfoAppState.new();
    }
  };
  (main._RecipeInfoAppState.new = function() {
    this.cuisines = T.JSArrayOfString().of(["American", "British", "Canadian", "Chinese", "Dutch", "Egyptian", "French", "Greek", "Indian", "Italian", "Japanese", "Mexican", "Moroccan", "Spanish", "Thai", "Vietnamese"]);
    this.selectedCuisine = null;
    this[_recipeData] = null;
    this[_isLoading] = false;
    main._RecipeInfoAppState.__proto__.new.call(this);
    ;
  }).prototype = main._RecipeInfoAppState.prototype;
  dart.addTypeTests(main._RecipeInfoAppState);
  dart.addTypeCaches(main._RecipeInfoAppState);
  dart.setMethodSignature(main._RecipeInfoAppState, () => ({
    __proto__: dart.getMethods(main._RecipeInfoAppState.__proto__),
    fetchRecipeInfo: dart.fnType(async.Future$(dart.void), [core.String]),
    fetchRecipeDetails: dart.fnType(async.Future$(core.Map$(core.String, dart.dynamic)), [core.String]),
    showRecipeDetails: dart.fnType(dart.void, [framework.BuildContext, core.Map$(core.String, dart.dynamic)]),
    build: dart.fnType(framework.Widget, [framework.BuildContext])
  }));
  dart.setLibraryUri(main._RecipeInfoAppState, I[0]);
  dart.setFieldSignature(main._RecipeInfoAppState, () => ({
    __proto__: dart.getFields(main._RecipeInfoAppState.__proto__),
    cuisines: dart.fieldType(core.List$(core.String)),
    selectedCuisine: dart.fieldType(dart.nullable(core.String)),
    [_recipeData]: dart.fieldType(dart.nullable(core.List)),
  ...
  var recipe$ = dart.privateName(recipe_details_page, "RecipeDetailsPage.recipe");
  recipe_details_page.RecipeDetailsPage = class RecipeDetailsPage extends framework.StatelessWidget {
    get recipe() {
      return this[recipe$];
    }
    set recipe(value) {
    ...
    static ['_#new#tearOff'](opts) {
      ...
      return new recipe_details_page.RecipeDetailsPage.new({recipe: recipe});
    }
    build(context) {
      let t0, t0$;
      return new scaffold.Scaffold.new({appBar: new app_bar.AppBar.new({title: new text.Text.new(core.String.as((t0 = this.recipe[$_get]("strMeal"), t0 == null ? "Nome da Receita Não Disponível" : t0)))}), body: new basic.Padding.new({padding: C[1] || CT.C1, child: new basic.Column.new({crossAxisAlignment: flex.CrossAxisAlignment.start, children: T.JSArrayOfWidget().of([new image.Image.network(core.String.as((t0$ = this.recipe[$_get]("strMealThumb"), t0$ == null ? "URL da Imagem Não Disponível" : t0$)), {width: 1 / 0, height: 200, fit: box_fit.BoxFit.cover}), new basic.SizedBox.new({height: 20}), new text.Text.new("Categoria: " + dart.str(this.recipe[$_get]("strCategory")), {style: new text_style.TextStyle.new({fontSize: 18})}), new basic.SizedBox.new({height: 10}), new text.Text.new("Instruções:", {style: new text_style.TextStyle.new({fontSize: 18, fontWeight: ui.FontWeight.bold})}), new basic.SizedBox.new({height: 10}), new basic.Expanded.new({child: new single_child_scroll_view.SingleChildScrollView.new({child: new text.Text.new(core.String.as(this.recipe[$_get]("strInstructions")), {style: new text_style.TextStyle.new({fontSize: 16})})})})])})})});
    }
  };
  (recipe_details_page.RecipeDetailsPage.new = function(opts) {
    let recipe = opts && 'recipe' in opts ? opts.recipe : null;
    this[recipe$] = recipe;
    recipe_details_page.RecipeDetailsPage.__proto__.new.call(this);
    ;
  }).prototype = recipe_details_page.RecipeDetailsPage.prototype;
  dart.addTypeTests(recipe_details_page.RecipeDetailsPage);
  dart.addTypeCaches(recipe_details_page.RecipeDetailsPage);
  dart.setMethodSignature(recipe_details_page.RecipeDetailsPage, () => ({
    __proto__: dart.getMethods(recipe_details_page.RecipeDetailsPage.__proto__),
    build: dart.fnType(framework.Widget, [framework.BuildContext])
  }));
  dart.setLibraryUri(recipe_details_page.RecipeDetailsPage, I[1]);
  dart.setFieldSignature(recipe_details_page.RecipeDetailsPage, () => ({
    __proto__: dart.getFields(recipe_details_page.RecipeDetailsPage.__proto__),
    recipe: dart.finalFieldType(core.Map$(core.String, dart.dynamic))
  }));
  ...
  dart.trackLibraries("zapp_user_main", {
    ...
    "file:///zapp/project/lib/recipe_details_page.dart": recipe_details_page,
  ...
  return {
    ...
    http: http,
    zapp__project__lib__recipe_details_page: recipe_details_page,
    src__client: client$,
